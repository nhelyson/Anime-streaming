    ...
    fetch(
      ...
      .then((data) => {
        ...
        if (receive.length > 0) {
          ...
          div.className =
            "border rounded-3 p-4 bg-white mx-auto mt-5 me-auto custom-padding";
          ...
          receive.forEach((Anime) => {
            ...
            fetch(
              ...
              .then((data) => {
                ...
                const traductionComplete = segments
            ...
            card.addEventListener("mouseenter", function () {
              ...
              if (!mouse) {
                ...
                div_mouse.className =
                  "card position-absolute mouse bg-light d-lg-block d-md-block d-sm-none d-none";
                div_mouse.style.zIndex = "1000000";
                div_mouse.style.top = `${
                  card.getBoundingClientRect().top + window.scrollY
                }px`;
                div_mouse.style.left = `${
                  card.getBoundingClientRect().right + 10
                }px`;
                ...
                title.innerHTML = `${
                  Anime.attributes.titles?.en || Anime.attributes.canonicalTitle
                } ${Anime.attributes.createdAt.substring(0, 4)}`;
            ...
            card.addEventListener("click", function () {
              ...
              let historique_recept =
                JSON.parse(localStorage.getItem("historique")) || [];
              historique_recept.unshift(history);
              localStorage.setItem(
                "historique",
                JSON.stringify(historique_recept)
              );

              window.location.href = `manga_info.html?Anime_name=${
                Anime.attributes.titles.en || Anime.attributes.canonicalTitle
              }
...
if (document.getElementById("title_Anime")) {
  ...
  console.log(page);
  document.title = `[${Anime_name}]<div><span class="fw-bold.">K</span>otsu</div>`;
  ...
  fetch(`https://kitsu.io/api/edge/anime/${encodeURIComponent(id)}`)
    ...
    .then((data) => {
      ...
      fetch(
        ...
        .catch((error) => {
          console.error("Erreur :", error);
          document.getElementById("resultat").inne =
            "Une erreur s'est produite.";
        });
      ...
      title.innerHTML = `[${
        Anime_search.attributes.titles.en ||
        Anime_search.attributes.canonicalTitle
      }]`;
      ...
      title.addEventListener("mouseenter", function () {
        ...
        alternative_name.className =
          "alternative_name  position-absolute bg-light p-5 rounded-2 mt-5";
        alternative_name.setAttribute(
          "style",
          "left:8rem; top:-2rem;z-index:1000000;"
        );
      ...
      const descript =
        Anime_search.attributes.synopsis || "Description indisponible";
...
`;
  ...
  fetch("https://graphql.anilist.co", {
    ...
    .then((data) => {
      ...
      video_trailer.setAttribute(
        "style",
        "overflow: hidden;width:16rem; height: 5rem;border-radius: 0.5rem!important;"
      );
    ...
    color:white!important;z-index:1000!important">trailer</p>`;
      ...
      news_anime.className =
        "mt-2 bg-white p-4 custom-well d-flex flex-lg-column flex-md-row flex-row custom-over";
      news_anime.style;
      let name_studios = "";
      ...
      data.data.Media.staff.edges.forEach((author_name) => {
        if (author_name.role) {
          if (
            author_name.role === "Original Creator" ||
            author_name.role === "Original Story"
          ) {
        ...
        const format_accept = ["TV"];
        if (
          !OriginalAuthor &&
          format_accept.some((format) =>
            data.data.Media.format.includes(format)
          )
        ) {
        ...
        if (
          !OriginalAuthor &&
          format_movie.some((format) => data.data.Media.format.includes(format))
        ) {
      ...
      data.data.Media.characters.edges.forEach((voice) => {
        ...
        voice.voiceActors.forEach((voice_man) => {
          const roles = ["Japanese"];
          if (
            roles.some((languageV2) =>
              voice_man.languageV2.includes(languageV2)
            )
          ) {
